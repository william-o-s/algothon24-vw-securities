...
def trade(S1_log: pd.Series, S2_log: pd.Series, S1_prices: pd.Series, S2_prices: pd.Series, beta: np.float64 = 1):
    stock1, stock2 = S1_log.name, S2_log.name
    ...
    # Latest price
    latest_price_s1 = S1_prices.iloc[-1]
    latest_price_s2 = S2_prices.iloc[-1]

    # Calculate maximum allowable positions based on $10,000 limit
    max_pos_S1 = (dlrPosLimit / latest_price_s1) * 0.5
    max_pos_S2 = (dlrPosLimit / latest_price_s2) * 0.5
...
def getMyPosition(prcSoFar):
    ...
    log_returns = np.log1p(prices.pct_change()).dropna()     # Convert to log returns
    (nt, nInst) = prcSoFar.shape

    if not pairs_cache:
        pairs_cache, _, beta_matrix = find_cointegrated_pairs(log_returns)
    ...
    # for (stock_1, stock_2) in [(0, 1)]:
        ...
        S1_log, S2_log = log_returns[stock_1].iloc[-rolling_window:], log_returns[stock_2].iloc[-rolling_window:]
        S1_prices, S2_prices = prices[stock_1].iloc[-rolling_window:], prices[stock_2].iloc[-rolling_window:]
        currentPos[stock_1], currentPos[stock_2], zscore, lower_band, lower_neutral_band, upper_neutral_band, upper_band = trade(S1_log, S2_log, S1_prices, S2_prices, beta_matrix[stock_1, stock_2])
